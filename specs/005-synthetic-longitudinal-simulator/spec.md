...
4. **Given** downstream dashboard or model specs request synthetic data, **when** no alternate generator is selected, **then** this longitudinal simulator is treated as the canonical synthetic source.
...
2. **Given** a generation run that misses a configured tolerance, **when** the summary is reviewed, **then** the miss is called out with the target, observed value, and tolerance.
3. **Given** downstream specs depend on synthetic data, **when** they inspect the run summary, **then** they can determine whether the cohort is suitable for dashboard or modeling use.
...
- **FR-009**: Default summer heat settings MUST be enabled from 2026-06-01, with baseline temperature 94 F, heat-wave probability 0.20, heat-wave temperature 108 F, and heat-index noise standard deviation 4 F.
...
- **FR-015**: Daily observations MUST include systolic blood pressure, diastolic blood pressure, heart rate, respiratory rate, skin temperature, weight, body water, sleep, steps, wear hours, and scale adherence when observed.
...
- **Participant-Day**: One daily monitoring state for a participant, including latent risk and observed vitals, activity, sleep, adherence, and missingness.
...
- The default study window is 12 weeks, with future specs able to extend the window through configuration.
